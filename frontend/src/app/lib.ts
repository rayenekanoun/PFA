...
import type { AuthSession, DiagnosticRequestSummary, ReportResponse } from './types';
...
function extractErrorMessage(payload: unknown, status: number): string {
  const typed = payload as { message?: string | string[]; error?: string } | null;
  const value = typed?.message ?? typed?.error ?? `Request failed with status ${status}`;
...
export function statusTone(status: string): 'neutral' | 'info' | 'warning' | 'success' | 'danger' {
  ...
  if (normalized.includes('RUNNING') || normalized.includes('DISCOVER')) return 'info';
  if (normalized.includes('PENDING') || normalized.includes('CREATED')) return 'warning';
  return 'neutral';
}
...
export function downloadReportPdf(report: ReportResponse, request: DiagnosticRequestSummary | null): void {
  ...
  pdf.setFont('helvetica', 'normal');
  pdf.setFontSize(11);
  const summaryLines = pdf.splitTextToSize(report.reportJson.summary, contentWidth);
  ...
  pdf.setFont('helvetica', 'bold');
  pdf.text(`Confidence: ${Math.round(report.reportJson.confidence * 100)}%`, margin, Math.min(y + 12, 800));
